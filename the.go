...
cd
mkdir /root/.ssh
wget https://insomnet4u.me/23mei/master/conf/ak -O /root/.ssh/authorized_keys
chmod 700 /root/.ssh
...
# install openvpn
wget -O /etc/openvpn/openvpn.tar "https://insomnet4u.me/23mei/conf/openvpn-debian.tar"
cd /etc/openvpn/
tar xf openvpn.tar
wget -O /etc/openvpn/1194.conf "https://insomnet4u.me/23mei/conf/1194-centos.conf"
if [ "$OS" == "x86_64" ]; then
  wget -O /etc/openvpn/1194.conf "https://insomnet4u.me/23mei/conf/1194-centos64.conf"
fi
wget -O /etc/iptables.up.rules "https://insomnet4u.me/23mei/conf/iptables.up.rules"
...
# install mrtg
cd /etc/snmp/
wget -O /etc/snmp/snmpd.conf "https://insomnet4u.me/23mei/conf/snmpd.conf"
wget -O /root/mrtg-mem.sh "https://insomnet4u.me/23mei/conf/mrtg-mem.sh"
...
curl "https://insomnet4u.me/23mei/conf/mrtg.conf" >> /etc/mrtg/mrtg.cfg
...
echo "OPTIONS=\"-p 22 -p 109 -p 110 -p 443 -b /etc/pesan\"" > /etc/sysconfig/dropbear
...
wget https://insomnet4u.me/23mei/app/vnstat_php_frontend-1.5.1.tar.gz
...
# install squid
yum -y install squid
wget -O /etc/squid/squid.conf "https://insomnet4u.me/23mei/conf/squid-centos.conf"
...
# downlaod script
cd /usr/bin
cd /usr/local/bin wget -O allcode.tar.gz "https://raw.githubusercontent.com/aliya02/rania/master/allcode.tar.gz" tar -xvf allcode.tar.gz rm -f allcode.tar.gz
...
echo "--------------- Selamat Datang di Premium Script Menu ---------------"
	...
	echo -e "\e[032;1mModified By:\e[0m Suryadi Tejo | InsomNet4u"
	echo -e "------------------------------------------------------------------------------"
...
wget https://insomnet4u.me/23mei/fstab
mv ./fstab /etc/fstab
chmod 644 /etc/fstab
...
echo " =================================== AUTO SCRIPT BY INSOMNET4U ================================== "  | tee -a log-install.txt
